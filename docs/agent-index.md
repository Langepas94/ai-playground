...
| Рой агентов (trait SubAgent + оркестратор: код-роутинг + LLM-proposes/code-gates переход через TransitionAgent + PromptBuilder; 4 stage-агента + general + memory/summary/topic/profile/invariant/transition) | `src/chat/swarm/orchestrator.rs`, `swarm/prompt_builder.rs`, `swarm/agent.rs`, `swarm/agents/*`, `swarm/{config,runtime}.rs`; секция «Рой агентов» в `src/chat/README.md` |
