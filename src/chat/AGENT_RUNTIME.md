...
Extractor обязан сохранять атомарные KV-факты. Нельзя сохранять целый prompt как
`preferences` только из-за слова “хочу”. Примеры нормальных KV:

- `location=Москва`;
- `location=Москва` из “москвич”;
- `age=999 лет`;
- `appearance_hair=зеленые волосы`;
