...
домен, свободный стартовый контекст, опциональные поля для уточнения, инварианты,
...
- **стартовый контекст** — свободное описание фактов, которые уже известны при
  создании агента; сохраняется как long-term fact `project_context` под
  `agent:<id>` и доступен во всех чатах этого агента.
