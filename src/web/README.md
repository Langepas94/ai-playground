...
- **долговременная** — `AgentProfile` (схема интервью + значения). Агент сам
  ...
  Стартовый контекст в формате `key = value` сохраняется отдельными заполненными
  profile-полями и отдельными long-term facts; свободный текст остаётся единым
  `project_context`.
- **рабочая** — `TaskContext` со стадией FSM (`clarify→planning→execution→
  ...
  Validation.
- **инварианты** — редактируемый список (вкладка `Инварианты`); это **код-проверка**,
  а не LLM-агент (детерминированный линтер ответа, без запроса к провайдеру).
  Стадийные правила («сначала план», «финал только после проверки») обеспечивает
  FSM. Правила, непроверяемые кодом, передаются модели как рекомендация и не
  блокируют ответ. UI показывает `PASS` / `BLOCKED` / `ADVISORY` summary. Для
  агентов с инвариантами streaming буферизуется до завершения проверки:
  непроверенные partial tokens браузеру не отправляются.
