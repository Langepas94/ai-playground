...
use crate::{
    ...
    config::{AppConfig, McpServerConfig, ProfileConfig},
    errors::AppError,
    mcp::{McpToolInfo, connect_and_list},
...
fn build_app(state: AppState) -> Router {
    Router::new()
        ...
        .route("/api/mcp/servers", get(mcp_servers).post(mcp_servers_save))
...
async fn mcp_servers() -> Result<Json<McpServersResponse>, WebError> {
    let config = AppConfig::load()?;
    Ok(Json(McpServersResponse {
        servers: mcp_server_views(&config),
    }))
}

/// Persist or delete a named MCP server in the config file so it survives page
/// reloads and appears in the configured-server list.
async fn mcp_servers_save(
    ApiJson(request): ApiJson<McpServerSaveRequest>,
) -> Result<Json<McpServersResponse>, WebError> {
    let name = request.name.trim().to_string();
    if name.is_empty() {
        return Err(AppError::InvalidInput("Server name is empty".to_string()).into());
    }
    ...
    if request.delete {
        ...
        let server = build_saved_server(&request)?;
...
fn mcp_server_views(config: &AppConfig) -> Vec<McpServerView> {
    config
        .mcp_servers
        .iter()
        .map(|(name, server)| match server {
            McpServerConfig::Stdio { command, args } => McpServerView {
                name: name.clone(),
                transport: "stdio".to_string(),
                detail: format!("{command} {}", args.join(" ")).trim().to_string(),
            },
            McpServerConfig::Http { url } => McpServerView {
                name: name.clone(),
...
/// Build the server definition to persist. Remote `url` wins over stdio
/// `command`, matching the connect precedence.
fn build_saved_server(request: &McpServerSaveRequest) -> Result<McpServerConfig, WebError> {
    if let Some(url) = blank_str_to_none(&request.url) {
        return Ok(McpServerConfig::Http {
            url: url.to_string(),
        });
    }
    if let Some(command) = blank_str_to_none(&request.command) {
    ...
    Err(
        AppError::InvalidInput("Provide a stdio command or a remote URL to save".to_string())
            .into(),
    )
}
...
/// Resolve a tools request into a concrete server. Precedence: explicit `url`,
/// then ad-hoc `command`, then a configured `server` name.
fn resolve_mcp_server(request: McpToolsRequest) -> Result<(String, McpServerConfig), WebError> {
    if let Some(url) = blank_to_none(request.url) {
        return Ok((url.clone(), McpServerConfig::Http { url }));
    }
    if let Some(command) = blank_to_none(request.command) {
        let label = format!("{command} {}", request.args.join(" "))
            .trim()
            .to_string();
        return Ok((
            label,
    ...
    if let Some(name) = blank_to_none(request.server) {
        let config = AppConfig::load()?;
        let server = config.mcp_server(&name)?.clone();
        return Ok((name, server));
    }
    Err(AppError::InvalidInput(
        "Provide a configured server name, a stdio command, or a remote URL".to_string(),
    )
    .into())
}
